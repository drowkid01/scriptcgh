...
EOF
echo " IP : $(wget -qO- ifconfig.me)" > /etc/VPS-MX/HYSTERIA/data.yaml
...
msg -bar
    ...
    red "$(cat /root/hy/url-nohop.txt)"
read -p "$(green "Hysteria 2 Modulos UDP By @botlatmx Finalizado ") "
}
...
#openssl req -new -x509 -days 3650 -key /etc/VPS-MX/HYSTERIA/udpmod.ca.key -subj "/C=CN/ST=GD/L=SZ/O=DrowKid, Inc./CNDrowKid Root CA" -out /etc/VPS-MX/HYSTERIA/udpmod.ca.crt
...
openssl req -x509 -new -nodes -key /etc/VPS-MX/HYSTERIA/udpmod.ca.key -days 3650 -out /etc/VPS-MX/HYSTERIA/udpmod.ca.crt -subj "/C=CN/ST=GD/L=SZ/O=botlatmx, Inc./CN=DrowKid Root CA"
openssl req -newkey rsa:2048 -nodes -keyout /etc/VPS-MX/HYSTERIA/udp.server.key -subj "/C=CN/ST=GD/L=SZ/O=botlatmx, Inc./CN=${domain}" -out /etc/VPS-MX/HYSTERIA/udpmod.server.csr
...
if wget -O /etc/VPS-MX/HYSTERIA/config.json https://raw.githubusercontent.com/drowkid01/scriptcgh/main/Recursos/menu_inst/UDPserver-sh/config.json &>/dev/null ; then
...
if wget -O /bin/authSSH https://raw.githubusercontent.com/drowkid01/scriptcgh/main/Recursos/menu_inst/UDPserver-sh/authSSH &>/dev/null ; then
		chmod +x /bin/authSSH
		msg -verd ' OK'
	else
		msg -verm2 ' FAIL '
...
if wget -O /etc/VPS-MX/HYSTERIA/hysteria.service https://raw.githubusercontent.com/drowkid01/scriptcgh/main/Recursos/menu_inst/UDPserver-sh/hysteria.service &>/dev/null ; then
...
unset port
  ...
  2) source <(curl -sSL https://raw.githubusercontent.com/drowkid01/scriptcgh/main/Recursos/test/udp-custom.sh) && exit;;
...
echo '[Unit]
Description=HysteriaUDP MOD Service BY @botlatmx
After=network.target

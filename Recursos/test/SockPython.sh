...
[[ -z ${texto_soket} ]] && texto_soket='<span style=color: #ff0000;><strong><span style="color: #ff0000;">b</span><span style="color: #ff9900;">o</span><span style="color: #008000;">t</span><span style="color: #0000ff;">l</span><span style="color: #ff0000;">a</span><span style="color: #ff9900;">t</span><span style="color: #008000;">m</span><span style="color: #0000ff;">x</span><span style="color: #ff0000;">.</span></strong></span>'
...
[[ -e /etc/VPS-MX/PDirect ]] && {
echo -e "[Unit]
Description=WS-Epro Service
...
ExecStart=/bin/WS-Epro -salome -listen :${porta_socket} -ssh 127.0.0.1:${local} -f /etc/VPS-MX/PDirect 
Restart=always
RestartSec=3s
...
- target_host: 127.0.0.1
  target_port: ${local}
  listen_port: ${porta_socket}" > /etc/VPS-MX/PDirect
...
User=root
WorkingDirectory=/root
ExecStart=/bin/WS-Epro -f /etc/VPS-MX/PDirect 
Restart=always
RestartSec=3s
...
def main(host=LISTENING_ADDR, port=LISTENING_PORT):
    ...
    print "\033[0;34m�01"*10,"\033[1;32m DrowKid ADM - LITE","\033[0;34m�01\033[1;37m"*11,"\n"
...
[[ ! -e /etc/VPS-MX/PortPD.log ]] && echo -e "${conf}" > /etc/VPS-MX/PortPD.log
}
